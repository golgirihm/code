...
Bullet::Bullet(QGraphicsRectItem *parentRectItem)
{
    // draw the bullet
    ...
    if(parentRectItem) setPos(parentRectItem->x() + (parentRectItem->rect().width() - rect().width())/2, parentRectItem->y() - rect().height() - PX_BULLET_PADDING);
}
