...
//        {
            ...
            buffer.resize(sizeof(cardstack));
            qDebug() << "resizing buffer to " <<  sizeof(cardstack) << "==" << buffer.size() << "bytes";

            memcpy(buffer.data(), &cs1, sizeof(cardstack));
            qDebug() << "copying " << sizeof(cardstack) << " bytes to buffer";

            memcpy(&cs2, buffer.data(), sizeof(cardstack));
            qDebug() << "copying " << buffer.size() << " bytes to msg2";



//            message msg1(C::LOBBYTEXT, "Some text from msg1", "from msg1 sender", fakecard);
//            message msg2(C::CARD_FOR_HAND, "TEXT", "FROM", card());

//            buffer.resize(sizeof(message));
//            qDebug() << "resizing buffer to " <<  sizeof(message) << "==" << buffer.size() << "bytes";

//            memcpy(buffer.data(), &msg1, sizeof(message));
//            qDebug() << "copying " << sizeof(message) << " bytes to buffer";

//            memcpy(&msg2, buffer.data(), sizeof(message));
//            qDebug() << "copying " << buffer.size() << " bytes to msg2";


//            qDebug() << "msg1 == " << msg1.msgtype() << msg1.text() << msg1.from() << msg1.sent_cardstack().TopCard().string();
//            qDebug() << "msg2 == "  << msg2.msgtype() << msg2.text() << msg2.from() << msg2.sent_cardstack().TopCard().string();
...
//                qDebug() << "from error";
//            if(msg2.sent_cardstack().TopCard().string() != msg1.sent_cardstack().TopCard().string())
...
void game::deal()
{
    ...
    for(int CardCount = 0; CardCount < INITIAL_CARD_QTY; ++CardCount)
    {
        ...
        for(int ClientNumber = 0; ClientNumber < NumClients; ++ClientNumber)
        {
            ...
            message msg(C::CARD_FOR_HAND, NO_STRING, NO_STRING, MainDeck->TakeTopCard());
